...
const TechStudyAnalytics = (() => {
  ...
  function _insightText(sessions, range, all) {
    const now  = new Date();
    const goal = AppState.techStudyDailyGoal || 240;
    ...
    const allTimeStr = activeDays > 0
      ...
      : 'No sessions logged yet — start your first techStudy session! 📖';
    ...
    if (range === 'today') {
      ...
      if (!activeDays) return 'No sessions logged yet — start your first techStudy session! 📖';
  ...
  function _insight(sessions, range, all) {
    const el = document.getElementById('tsaInsight');
    if (!el) return;
  ...
  function _heatmap() {
    const el = document.getElementById('tsaHeatmap');
    if (!el) return;
    ...
    // Streak count
    const cnt = document.getElementById('tsaStreakCount');
    if (cnt) {
      let streak = 0;
      const d = new Date();
  ...
  function _comparison(sessions, range, all) {
    const el = document.getElementById('tsaComparisonRow');
    if (!el) return;
    ...
    const goalMin  = AppState.techStudyDailyGoal || 240;
    ...
    el.innerHTML =
      _card('TechStudy Time', _fmtDur(curMin), prevMin ? _fmtDur(prevMin) : 0) +
  ...
  function _trend(sessions, range, all) {
    const canvas = document.getElementById('tsaTrendChart');
    if (!canvas || typeof Chart === 'undefined') return;
    if (_trendChart) { _trendChart.destroy(); _trendChart = null; }

    const prev   = _prevPeriod(range, all);
    const theme  = T();
    const titleEl = document.getElementById('tsaTrendTitle');
    ...
    _trendChart = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [
          ...
          {
            label: range === 'today' ? 'Today' : 'TechStudy time',
            data: curData,
  ...
  function _hours(sessions, range, all) {
    const canvas = document.getElementById('tsaHourChart');
    if (!canvas || typeof Chart === 'undefined') return;
  ...
  function _donut(canvasId, legendId, sessions, key) {
    ...
    const labels  = sorted.map(([k]) => key === 'activity' ? TechStudyTracker.getActivity(k)?.label || k : TechStudyTracker.getSubject(k)?.label || k);
    const data    = sorted.map(([, v]) => v);
    const colors  = sorted.map(([k], i) => key === 'subject' ? (TechStudyTracker.getSubject(k)?.color || PALETTE[i % PALETTE.length]) : PALETTE[i % PALETTE.length]);
  ...
  return {
    ...
    init() {
      // Notify user if they leave the tab during a techStudy session
      document.addEventListener('visibilitychange', () => {
        if (!TechStudyTracker._running) return;
        const inBreak = TechStudyTracker._mode === 'pomodoro' &&
          (TechStudyTracker._pomoPhase === 'break' || TechStudyTracker._pomoPhase === 'longbreak');
        if (inBreak) return;
        ...
        if (document.hidden) {
          ...
          if (secs >= 10 && typeof UI !== 'undefined') {
            UI.showToast(`👀 Away ${TechStudyTracker.fmtTimer(secs)} — timer kept running.`);
          }
        }
      });
    },
    ...
    render(sessions, range) {
      ...
      _donut('tsaActivityChart', 'tsaActivityLegend', sessions, 'activity');
      _donut('tsaSubjectChart',  'tsaSubjectLegend',  sessions, 'subject');
    },
...
// Expose to window so `if (window.TechStudyAnalytics)` guards work
// (const at script top-level does NOT create window properties)
window.TechStudyAnalytics = TechStudyAnalytics;
