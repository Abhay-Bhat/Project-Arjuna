...
21. **Cloud sync from Android** — while signed in, make a change (e.g. check off a task), then *immediately* background the app (home button / recent-apps switch) within a second or two, without waiting. Reopen the app on a different signed-in device (or the web app) and confirm the change is present. This exercises the native background-flush fix — if data is still missing, check `chrome://inspect` (USB debugging) → the app's WebView console → run `SKADI.syncDiag()` to see whether `enabled`/`listenerActive` are `true` and whether the Firestore read throws `permission-denied` (which would point to a Firebase Console-side App Check setting, not something fixable in this codebase).
